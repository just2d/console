...
sql_file=$dir_tmp/month_whole_site_stat.sql
rs_file=$dir_tmp/month_whole_site_stat.rs

echo "
-- 设置时间变量
SET @start_date = DATE_FORMAT(DATE_SUB($cur_date,  INTERVAL 1 DAY), '%Y%m01');
SET @end_date = DATE_FORMAT($cur_date, '%Y%m%d');
...
WHERE s.month_no = t.month_no;" > $sql_file

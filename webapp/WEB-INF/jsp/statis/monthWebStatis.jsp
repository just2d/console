...
function getChangeStatis(){
	...
	if(type=="day"){
		...
		window.location.href="/statis/webStatis/getDayCount" ;
	}
}
...
<form action="/statis/webStatis/getMonthCount" method="post" id="webStatisForm">
...
<table>
	<tr>
		...
		<td>日期:</td><td><input id="startDate" class="def" name="date" style="width:100px;" class="dateCss" type="text" onclick="SelectDate(this,'yyyy-MM','')" readonly="readonly" value="${date}"/></td>
	</c:if>
	<c:if test="${param.type!='city' }">
		...
		<td>开始日期:</td><td><input id="startDate" class="def" name="startDate" style="width:100px;" class="dateCss" type="text" onclick="SelectDate(this,'yyyy-MM','')" readonly="readonly" value="${startDate}"/></td>
		<td>结束日期:</td><td><input id="endDate" class="def" name="endDate" style="width:100px;" class="dateCss" type="text" onclick="SelectDate(this,'yyyy-MM','')" readonly="readonly" value="${endDate}"/></td>
	</c:if>
...
<span id="changeSpan" >
	<span id="sp1" class="stat_two"   type="day">每日</span> 
	<span id="sp2" class="stat_two stat_high"  type="month">每月</span> 
</span>	
...
<div class="mag">
	<div class="todo">
		<table id="statTable">
				...
				<tbody>
				<c:forEach items="${monthWebStatisList }" var="monthWebStatis">
					<tr>
					<td><c:if test="${monthWebStatis.cityId==null ||monthWebStatis.cityId=='' || monthWebStatis.cityId=='-1'}">全国</c:if ><c:if test="${monthWebStatis.cityId!=null ||monthWebStatis.cityId!='' || monthWebStatis.cityId!='-1'}">${monthWebStatis.cityName }</c:if></td>
					<td>${monthWebStatis.month}</td>
					<td>${monthWebStatis.freeAgentNum}</td>
					<td>${monthWebStatis.payAgentNum}</td>
					<td>${monthWebStatis.verifyAgentNum}</td>
					<td>${monthWebStatis.onlineResaleNum}</td>
					<td>${monthWebStatis.onlineRentNum}</td>
					<td>${monthWebStatis.pubResaleNum}</td>
					<td>${monthWebStatis.pubRentNum}</td>
					<td>${monthWebStatis.loginUserNum}</td>
					<td>${monthWebStatis.pubHouseAgentNum}</td>
				</c:forEach>
				</tbody>

...
function confirmsave(){
	$.ajax({
		type : "post",
		async : false,
		url : "/estate/mapping/confirmsave",
		dataType : "text",
		data: $("#estateMappingForm").serialize(),
		success : function(back) {
			var data = eval("("+back+")");
...
<div style="width: 100%; text-align: left;">当前位置：小区映射添加 </div>
<br />

<form action="/estate/mapping/save" method="post" id="estateMappingForm" >
<input type="hidden" id="estateid" name="estateId"  />
<input type="hidden" id="distId" name="estateDistId"  />
<input type="hidden" id="blockId" name="estateBlockId"  />
	<div id="addExpert" class="todo">
		<table>
			...
			<tr>
				<td>联盟小区ID</td>
				<td><input type="text" id="uEstateId"" name="uEstateId"   onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></td>
				<td>来源</td>
				<td><select name="sourceId"><option value="1">58</option>
						<option value="13">安居客</option>
				</select>
				</td>
			</tr>

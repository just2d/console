...
<div style="width: 100%;text-align: left;">当前位置：房源管理 &gt; 二手房管理 &gt; 二手房管理</div>  
<br />
 <div id="tabs" class="tabs">  

     <ul>  

        <li ><a href="/resale/verify/allresale/list" style="background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%" >二手房管理</a></li>  
        <li class="tabs_active"  ><a href="/resale/verify/baseInfoList" >标题描述审核</a></li>  
...
<br />
	...
	<form action="/resale/verify/baseInfoList" method="post" id="resaleForm" >
						...
						<c:forEach items="${resaleList}" var="list">
							<tr style=" width:12.5%; text-align:center;" class="todo">
								<th colspan="2"><span style=" width:12.5%;float:left;padding-left:10px;">
									 ...
									 <!-- 防注释 -->
									</span><a style=" width:12.5%;float:right" href="http://${func:getCode(list.cityId)}.${applicationScope.sysDomain}/ershoufang/${list.houseId}-0.html" target="_blank">房源ID：${list.houseId}</a>
								</th>
							</tr>
							<tr onclick="triggerCheckbox(this);">
								<td style=" width:12.5%; text-align:center" colspan="2">
			                            ...
			                            <c:if test="${list.description.length() > 0 }">
			                               <iframe src="/resale/getDescription/${list.houseId}" id="description${list.houseId}" width="100%" scrolling="no" frameborder="0" ></iframe>
			                               <script type="text/javascript"> 
	...
	<div  style="text-align: center;" >
		...
		<c:if test="${empty resaleList }">
			<h2>已经审核完该城市二手房房房源的标题和描述信息,请选择其他城市审核或者返回.</h2>
		</c:if>
...
function refreshPage(){
	$("#resaleForm").attr("action","/resale/verify/refreshBaseInfoPage");
	document.getElementById('resaleForm').submit();
}

...
<div style="width: 100%;text-align: left;">当前位置：房源管理 &gt; 二手房管理 &gt; 房源管理</div>  
  <br />
 <div id="tabs" class="tabs">  

     <ul>  

        <li class="tabs_active" ><a href="/resale/verify/allresale/list"  >二手房管理</a></li>  
        ...
        <li ><a href="/resale/verify/list/verifyPhoto/1" style="background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%" >户型图审核</a></li>  
        <li ><a href="/resale/verify/list/verifyPhoto/2" style="background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%" >室内图审核</a></li>  
        <li ><a href="/resale/verify/list/verifyPhoto/3" style="background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%" >小区图审核</a></li>  

     </ul>
     </div>
     <form action="/resale/verify/allresale/list" method="post" id="resaleForm">
  <div class="mag">
     <br />
     <div class="search" >
<table>
				...
				<table>
					...
					<thead>
						...
						<tr>
							...
							<th>现有分值</th>
							<c:if test="${sessionScope.sessionUser.isPass('/resale/addExtraScore') }">
					...
					<tbody class="rasent">
						...
						<c:forEach items="${resaleList}" var="list">
								...
								<td style=" text-align:center"><a href="http://${list.cityDir}.${applicationScope.sysDomain}/ershoufang/${list.id}-0.html" target="_blank">${list.id}</a></td>
								
									<td style=" text-align:center"><a href="javascript:postform('/agentManage/search',{type:'id',searchtxt:'${list.authorid}'});" >${list.authorid}</a></td>
									...
									<td style=" text-align:center">${list.authorMobile}
									...
									<td style="text-align:center" >
									<c:if test="${list.hxNum>0  || sessionScope.sessionUser.isPass('/verify/allrent/list/audit')}" >
									...
									<c:if test="${list.snNum>0 || sessionScope.sessionUser.isPass('/verify/allrent/list/audit')}" >
									...
									<br/>
									<c:if test="${ list.xqNum>0 || sessionScope.sessionUser.isPass('/verify/allrent/list/audit')}" >
									...
									<td style="text-align:center" >
										<c:choose>
											<c:when test="${list.treated&&!list.approvaled}"> 违规  
											</c:when>
											<c:otherwise>
												<a href="javascript:openReject(${list.id})" > [违规]</a> 
											</c:otherwise>
										</c:choose> &nbsp;
										<c:choose>
											<c:when test="${list.approvaled&&list.treated }" > 已通过
											</c:when>
											<c:otherwise> 
												<a href="javascript:pass(${list.id})" >[通过]</a>
											</c:otherwise>
										</c:choose> &nbsp;
									</td>
									</c:if>
...
function showExrtaScoreHistory(id){
	...
	$("#divExtraScoreHistory").dialog("option", "position", "center");
		...
		$.ajax({
			type : "post",
			async : false,
			url : "/resale/getExtraScoreHistory/"+id,
			dataType : "json",
			success : function(data) {
...
function saveExtraScore(){
	...
	var id = $("#extraScoreResaleId").html();
...
<table>
	<tr>
		...
		<td style="text-align: left">
		<label id="extraScoreResaleId"></label>
		</td>
	</tr>
	<tr>
		<td style="text-align: left" valign="top">

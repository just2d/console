...
<div style="width: 100%;text-align: left;">当前位置${houseList}：房源管理 &gt; 出租房管理 &gt; ${photoType==""?'图片':photoType }审核</div>  
<br />
 <div id="tabs" class="tabs">  

     <ul>  

         <li style="background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%" ><a href="/rent/verify/allrent/list" >租房管理</a></li> 
         ...
         <li class="${type==1?'tabs_active':'' }" style="${type==1?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/rent/verify/list/verifyPhoto/1">户型图审核</a></li>
         <li class="${type==2?'tabs_active':'' }" style="${type==2?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/rent/verify/list/verifyPhoto/2">室内图审核</a></li>
         <li class="${type==3?'tabs_active':'' }" style="${type==3?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/rent/verify/list/verifyPhoto/3">小区图审核</a></li>
     </ul> 
 </div> 
	<br />
	
	<div class="mag">
		<label>所在城市:</label> <select id="city" name="cityid"
			...
			<c:forEach items="${applicationScope.simpleLocaleMap }" var="entry">
				<c:if test="${cityid==entry.key }">
					<option value="${entry.key }" selected="selected">${entry.value.code
						}${entry.value.name }</option>
				</c:if>
				<c:if test="${cityid!=entry.key }">
					<option value="${entry.key }">${entry.value.code
						}${entry.value.name }</option>
				</c:if>
			</c:forEach>
		...
		<label
			id="passedPhotoCount"
			style="height: 20px; width: 1px; background-color: #AFE4D0;"></label>
		<input type="button" value="查看今日已审图片数量"
			onclick="getPassedPhotoCount(this);" /> <label id="awaitCount"
			style="height: 20px; width: 1px; background-color: #AFE4D0;"></label>
		<input type="button" value="待审${photoType }房源数量"
			onclick="getAllCount(this);" />&nbsp;&nbsp;&nbsp;<label id="notPassedPhotoCount">${notPassedPhotoCount}</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<div class="mag">
		<div class="todo">
			<table>
				...
				<tbody class="rasent">
					<tr>
						<c:forEach items="${rentList}" var="list" varStatus="status">
							<c:if test="${(status.count-1)%5==0&&status.count>1 }">
								</tr><tr>
							</c:if>
							<td class="image_wrap" style="color: black" id="${list.houseId }_${list.id}_${list.rentSmallPhotoUrl}">
								<img src="${list.rentSmallPhotoUrl }" width="100px"
								height="75px" onerror="this.src='${applicationScope.headPrefix}/m/house_m.png'" />
								<br /> 
								<br/>
								<input type="button" value="查看大图" onclick="showImg('${list.rentLargePhotoUrl}')"/>
								<br/>
								<c:if test="${type==1 }"> 
									<br/> <input id="isDirection_${list.id}" type="checkbox" <c:if test="${list.category==11||list.category==12}">checked='checked'</c:if> >有方向指示</input>
								</c:if> 
								<c:if test="${type==2 }">${list.coverFlag }</c:if> 
								<input value="" type="hidden" id="${list.houseId }_${list.id}_${list.rentSmallPhotoUrl}_reason" />
								<br/><input type="checkbox" name="_select_pictures" id="_select_pictures" valuetype="${list.houseId }_${list.id}_${list.rentSmallPhotoUrl}" onclick="resetCss('${list.houseId }_${list.id}_${list.rentSmallPhotoUrl}')" />
								<input value="${list.houseId }_${list.id}_${list.rentSmallPhotoUrl}" type="hidden" id="check_row_${status.count}" />
							</td>
						...
						<c:if test="${rentList.size()>5}">
						<c:choose>
							<c:when test="${rentList.size() >0 && (rentList.size())%5!=0 && (rentList.size())%5==1 }">
								<td></td><td></td><td></td><td></td><td><input type="checkbox" name="chk_row" id="chk_row" onclick="javascript:check_row(this.checked,'${rentList.size()+4}')" /></td>
							</c:when>
							<c:when test="${rentList.size() >0 && (rentList.size())%5!=0 && (rentList.size())%5==2 }">
								<td></td><td></td><td></td><td><input type="checkbox" name="chk_row" id="chk_row" onclick="javascript:check_row(this.checked,'${rentList.size()+3}')" /></td>
							</c:when>
							<c:when test="${rentList.size()>0 && (rentList.size())%5!=0 && (rentList.size())%5==3 }">
								<td></td><td></td><td><input type="checkbox" name="chk_row" id="chk_row" onclick="javascript:check_row(this.checked,'${rentList.size()+2}')" /></td>
							</c:when>
							<c:when test="${rentList.size() >0&& (rentList.size())%5!=0 && (rentList.size())%5==4 }">
								<td></td><td><input type="checkbox" name="chk_row" id="chk_row" onclick="javascript:check_row(this.checked,'${rentList.size()+1}')" /></td>
							</c:when>
						</c:choose>
						</c:if>
					</tr>
	...
	<div  style="text-align: center;" >
		...
		<c:if test="${empty rentList && empty circleCount }">
			<h2>已经审核完该城市出租房房源的<font color="red">${photoType }</font>图片,请选择其他类型图片审核或者返回.</h2>
		</c:if>
		<c:if test="${empty rentList && not empty circleCount }">
...
function requestPage(){
	window.location.href="/rent/verify/list/verifyPhoto/${type}?cityid=${cityid }";
}
...
function changeCity(cityid){
	// 图片类型
	var type = '${type}';
	window.location.href="/rent/verify/list/verifyPhoto/"+type+"?ranNum="+ranNum+"&cityid="+cityid;
}

...
<div style="width: 100%;text-align: left;">当前位置：租房星级评价管理</div> 

<form action="/rent/eval/list" method="post" id="rentForm" style="margin-top:0px;margin-top:0px;">
	...
	<div class="todo">
		<table id="rentTable" class="c" style="text-align:center">
			<thead>
				<tr>
			...
			<c:choose>
				<c:when test="${not empty rents}">
					<c:forEach items="${rents}" var="rent">
						<tr>
							<td><a target="_blank" href="http://${func:getCode(rent.cityid)}.taofang.com/zufang/${rent.id}-${rent.sourceId}.html">${rent.id}</a></td>
							<td>${rent.cityName}</td>
							<td>${rent.authorid}</td>
							<td>${rent.author}</td>
							<td>${rent.authorPhone}</td>
							<td>${rent.evaluationCount}</td>
							<td>${rent.evaluationPoint}</td>
							<td>
								<c:if test="${rent.sourceId==0 && rent.falseHouseReport==2 && rent.appealExpiredTime != null }">
								<a href="javascript:realHouse(${rent.id },${rent.sourceId },1);">真实房源</a>-
								<a href="javascript:realHouse(${rent.id },${rent.sourceId },2);">虚假房源</a>-
								</c:if>
								<a href="/rent/eval/evalList/${rent.id}/${rent.sourceId}" target="_blank">删除部分评价</a>-
								<span id="span${rent.id}">
									<c:choose>
										<c:when test="${rent.evaluationVisible == 0}">
											<a href="javascript:changeVisibleStatus('${rent.id}','${sourceId}','1');">开启评价</a>
										</c:when>
										<c:otherwise>
											<a href="javascript:changeVisibleStatus('${rent.id}','${sourceId}','0');">关闭评价</a>
										</c:otherwise>
									</c:choose>	
								</span>
							</td>
...
<script type="text/javascript">
	...
	function changeVisibleStatus(houseId, sourceId, status) {
		$.ajax({
			type : "GET",
			async : false,
			url : "/rent/eval/changeVisible/" + houseId + "/" + sourceId + "/" + status,
			dataType : "json",
			success : function(data) {
				if(data.result) {
					if(status == 1) {
	...
	function realHouse(houseId, sourceId, status) {
		$.ajax({
			type : "GET",
			async : false,
			url : "/rent/eval/realHouse/" + houseId + "/" + sourceId + "/" + status,
			dataType : "json",
			success : function(data) {
				if(data.result) {
					alert("操作成功");

	...
	function openDialog(str) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (res.auditStep == 1||res.auditStep == 2||res.auditStep == 3||res.auditStep == 5||res.auditStep == 6) {
					if(id_url[1]==''){
				...
				if (res.auditStep == 4) {
					$("#title").html(res.extra == undefined ? '' : "<b>房源标题:</b><a>" + res.extra+"</a>");
...
<body>
	<div id="tabs" class="tabs">
		...
		<ul>
			...
			<li class="${type==1?'tabs_active':'' }" style="${type==1?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/auditHistory/list/1">标题描述审核记录</a></li>
			<li class="${type==2?'tabs_active':'' }" style="${type==2?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/auditHistory/list/2">图片审核记录</a></li>
			<li class="${type==3?'tabs_active':'' }" style="${type==3?'':'background:url(/images/ui-bg_glass_75_e6e6e6_1x400.png) #e6e6e6 repeat-x 50% 50%'}"><a href="/auditHistory/list/3">经纪人审核记录</a></li>
	...
	<form action="/auditHistory/list/${type }" method="post" id="form">
		<div class="mag" style="padding-top: 10px;">
			ID：<input name="houseid" value="${houseid }" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" /> 经纪人ID：<input name="authorOrId" value="${authorOrId }" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" /> 处理人名字：<input name="dealerName" value="${dealerName }" /> <br></br>时间：<input name="startTime" value="${startTime }" style="width: 80px;"
			...
			<table width="100%">
				...
				<c:forEach items="${list }" var="item">
					<tr>
						...
						<td>${item.authorId }</td>
						<td><a href="/auditHistory/list/1?userId=${item.dealerId }" target="_blank">${item.dealerName }</a></td>
...
<div id="toShowPicDiv" style="display: none">
	<img id="toShowPicImg"></img><br></br> <font id="photoInfo"></font><br/><font id="photoRejectInfo" color="red"></font><br/>
</div>
	...
	<table>
		<tr>
			...
			<td style="text-align: left" valign="top"><c:forEach items="${reasons}" var="list" varStatus="vs">
					<input type="checkbox" id="batchReasons_${vs.index+1 }"><label for="batchReasons_${vs.index+1 }" id="batchReasons_${vs.index+1 }_value">${list.reason}</label><br />
				</c:forEach></td>
		</tr>
		<tr>

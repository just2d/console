...
function confirmDelEstate(){
	...
	var url = "${contextPath}/estate/authAction?estateId="+estateId+"&authStatus="+auth_Status+"&time="+new Date();
	//逻辑删除操作
		if(auth_Status == "3"){
...
function delEstate(resaleCount,rentCount,photocnt,layoutcnt,estateId){
	if(resaleCount > 0 || rentCount >0 ){
		alert("二手房或出租房数据不为空,不可删除");
		return;
	 }else{
		$.getJSON("${contextPath}/estate/deleteAction?estateId="+estateId+"&time="+new Date(), null, function(json){
		if(json!=null&&json.length>0){
	  		alert("删除成功");
	  		clear("form_estate_id");
	  		$("#estateListForm").submit();
...
function viewDetail(rentCount,resaleCount,estateId){
	url = "${contextPath}/estate/form?id="+estateId+"&action=detail&authStatus=${authStatus}";
	if(rentCount > 0 || resaleCount >0){
		url += "&flag=0";//不可删除
	}else{
		url += "&flag=1";//可以删除
	}
...
<input type="hidden" name="authStatus" value="2"/>
			...
			<tbody>
				<c:forEach items="${estateList}" var="estate">
					<tr onclick="setName('${estate.estateName}','form_estate_id');">
						...
						<td>
						<a href="javascript:void(0)" onclick="auth('${estate.estateId}','3')">删除</a>|<a href="javascript:void(0)" onclick="viewDetail('${estate.resaleCount}','${estate.rentCount}','${estate.estateId}')">查看</a>
						</td>
					</tr>
				</c:forEach>
